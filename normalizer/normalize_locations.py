...
class NormalizeLocation:
    def __init__(self, gadm_path: str, unsd_path: str):
        ...
        requests_cache.install_cache(
            self.geopy_cache_path,
            allowable_methods=["GET"],
            allowable_codes=[200],
            filter_fn=self._rate_limiter,
        )
        geolocator = Nominatim(
            user_agent="wikimpacts - impactdb; beta. Github: VUB-HYDR/Wikimpacts"
        )
        ...
        for col in self.unsd.columns:
            if "Code" not in col:
                self.unsd[col] = self.unsd[col].apply(
        ...
        try:
            try:
                if area:
                    assert isinstance(
                        area, str
                    ), f"Area `{area}` is not a string; type: {type(area)}"
                if re.match(
                    ...
                    self.logger.error(
                        f"Input `{area}` of type {type(area)} is not a valid area name"
                    )
                    return (None, None, None)
                if in_country:
                    assert isinstance(
                        in_country, str
                    ), f"Country is not a string: {in_country}"
                assert not (
            ...
            unsd_search_output = (
                self._get_unsd_region(area, return_name=True)
                if area and is_country
                else None
            )
            ...
            if is_country:
                ...
                query = area
                try:
                    country_codes = pycountry.countries.search_fuzzy(in_country)[
                        0
                    ].alpha_2
                except BaseException as err:
                    self.logger.debug(
            ...
            if not l or any(
                [
                    r.raw["type"].lower().strip() in self.unwanted_location_types
                    for r in l
                ]
            ):
            ...
            for result in l:
                ...
                if result.raw["geojson"]["type"].strip().lower() != "point":
                    ...
                    # prefer non-commercial locations (avoid matching shops/stores/offices)
                        ...
                        result.raw["type"].lower().strip()
                        not in self.unwanted_location_types
                    ):
                        location = result
                        break
                else:
            ...
            if us_area:
                ...
                # return the international name or wikipedia title if present (only for sublocations, due to GADM's format)
                    ...
                    "wikipedia" in location.raw["extratags"].keys()
                    and "en:" in location.raw["extratags"]["wikipedia"]
                ):
                    normalized_area_name = (
                        location.raw["extratags"]["wikipedia"]
                        .replace("en:", "")
                        .strip()
                    )
                else:
            ...
            geojson = (
                json.dumps(location.raw["geojson"])
                if isinstance(location.raw["geojson"], dict)
                else None
            )
            return (
                normalized_area_name,
                f'{location.raw["type"]}:{location.raw["addresstype"]}',
                geojson,
            )
        ...
        for level, region_list in regions.items():
            if area in region_list:
                return (
                    area
                    if return_name
                    else self.unsd.loc[self.unsd[level] == area][self.iso]
                    .unique()
                    .tolist()
                )
            else:
                ...
                if fuzzy_area_match:
                    return (
                        fuzzy_area_match[0]
                        if return_name
                        else self.unsd.loc[self.unsd[level] == fuzzy_area_match[0]][
                            self.iso
                        ]
                        .unique()
                        .tolist()
    ...
    def _get_american_area(self, area: str) -> list | None:
        ...
        address = (
            [x.strip() for x in area.split(",")]
            if area
            else [x.strip() for x in area.split(",")]
        )
    ...
    def get_gadm_gid(
        ...
        if country and not area:
            ...
            us_search = (
                ...
                if (self.united_states in country and self.united_states in area)
                    ...
                    country
                    if not area
                    else (f"{area}, {country}" if country and area else None)
                )
            )
        ...
        if country and area:
            unique_area_sets = [
                gadm_df[f"NAME_{l}"].dropna().unique().tolist() for l in range(1, 6)
            ]
            for cutoff in [0.75, 0.70]:
                level = 1
                for area_set in unique_area_sets:
                    closest_match = difflib.get_close_matches(
                        area, area_set, n=1, cutoff=cutoff
                    )
                    if closest_match:
                        return (
                            gadm_df.loc[gadm_df[f"NAME_{level}"] == closest_match[0]][
                                f"GID_{level}"
                            ]
                            .unique()
                            .tolist()
        ...
        for level in range(1, 6):
            ...
            if isinstance(area, str):
                alt_name = re.sub(
                    r"(county)|(city)|(prefecture)|(district)|(city of)|(region)",
                    "",
                    area,
                    flags=re.IGNORECASE,
                ).strip()
        ...
        for level in range(1, 5):
            ...
            varnames_list = [
                x for x in gadm_df[varname_col].to_list() if isinstance(x, str)
            ]
            ...
            for varnames in varnames_list:
                if area in varnames:
                    return (
                        gadm_df.loc[
                            gadm_df[varname_col].apply(
                                lambda x: (
                                    True if isinstance(x, str) and area in x else False
                                )
                            )
                        ][gid_col]
                        .unique()
    ...
    def _rate_limiter(
        ...
        if type(response) == requests_cache.models.response.CachedResponse:
            ...
            self.logger.debug(
                f"Ratelimiting by 1 second. Response type: {type(response)}"
            )
            sleep(1)
        return True
